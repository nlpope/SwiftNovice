...
class NetworkManager
{
    ...
    // look into Kitura
    private let coursesURL = "http://127.0.0.1:8080/"
    private let projectsURL = "http://127.0.0.1:8080/"
    ...
    {
        guard let url = URL(string: "\(coursesURL)")
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            guard let data
            else { completed(.failure(.invalidData)); return }
            
            do {
                let decoder = JSONDecoder()
                var prerequisites = try decoder.decode([SNCourse].self, from: data)
                completed(.success(prerequisites.sorted { $0.index < $1.index }))
            } catch {
    ...
    {
        guard let url = URL(string: "\(projectsURL)")
        else { completed(.failure(.invalidURL)); return }
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                completed(.success(projects.sorted { $0.index < $1.index }))
            } catch {
                completed(.failure(.invalidData))
            }
        }
